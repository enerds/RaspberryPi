...
toplogotm   = 5;        // 12  15  topmargin of logo
toplogohole = true;
...
inside_h      = 17.0;           // 12.1 = lowprofile    16.5 is full height
pcb_h         = 6.5;            // height needed for SD holder and solder points of pcb
...
//TOBI: inside_l = 86.0;                //85.00 is largest length reported, so a bit room on each side
inside_l = 90;
...
Chdmi_x=-1.2;
Chdmi_y=33.0;               // 32.4  if 15.1
Chdmi_w=11.8;
Chdmi_d=16.1;				  // 15.7 15.3
Chdmi_h=7.0;

Csnd_x=inside_w - 11.4;
Csnd_y=14.0;
Csnd_w=9.4; // TOBI: was 12.4
Csnd_d=12.0;
Csnd_h=10; // TOBI: was 13;
Csnd_r=6.7;
Csnd_z=3.0;
Csnd_l=3.4;

Crca_x=inside_w - 10.0 - 2.1;
Crca_y=33.3;                // 34.6 was 35.2
Crca_w=12.0;
Crca_d=12.4;                 // 9.8
Crca_h=22.0;
Crca_r=9.6;
    ...
    translate(v = [(box_w-w)/2, (box_l-d)/2,z]) {
        color(CASEcolor) {
            ...
            difference() {
                ...
                if (holes && !top) {
                    translate(v=[(w-hole_w)/2,holeofs,-0.1]) make_deckholes(noholes,hole_w,1.5,4,holeangle1);
                }
                if (logo) {
                    if (toplogohole) {
                        if (!topholes) {
...
module make_topcomponents() {
    color(CONNcolor) translate(v=[box_w1,box_l1,pcb_c]) {
        ...
        //SND
        translate(v=[Csnd_x,Csnd_y,0]) cube([Csnd_w,Csnd_d,Csnd_h],center=false);
        translate(v=[Csnd_x+Csnd_w,Csnd_y+Csnd_d/2,Csnd_z+Csnd_r/2]) rotate(a=90,v=[0,1,0]) cylinder(r=Csnd_r/2, h=Csnd_l, $fn=100);

        //RCA
        translate(v=[Crca_x,Crca_y,0]) cube([Crca_w,Crca_d,Crca_h],center=false);
        translate(v=[Crca_x+Crca_w,Crca_y+Crca_d/2,Crca_z+Crca_r/2]) rotate(a=90,v=[0,1,0]) cylinder(r=Crca_r/2, h=Crca_l, $fn=100);
    }
}
...
module make_connholes() {
    ...
    //set_cutout(box_w1+Cpwr_x,  box_l1+Cpwr_y +2, pcb_c - 5,   Cpwr_w, Cpwr_d+4, Cpwr_h+0.01);
    set_cutout(box_w1+Cpwr_x,  box_l1+Cpwr_y +4.05, pcb_c - 3.5,   Cpwr_w, Cpwr_d+4, Cpwr_h+0.01);
    ...
    set_cutout(box_w1+Csnd_x,box_l1+Csnd_y,pcb_c+2,    Csnd_w+7.2,Csnd_d,Csnd_h-2);

   // EXPANSION CABLE: width: 32.5 mm width, 2.mm height
   set_cutout(box_w1+Crca_x, box_l1 + Crca_y + 36 / 2, pcb_c + 7.0, Csnd_w+7, 36, 3, 0.1);
...
module remove_bottom() {
    ...
    translate(v = [-1, 91, -1])     cube([split2+1.1 , box_l, pcb_c+Cpwr_h+1.1], center = false);
        ...
        union() {
            ...
            color(CASEcolor) {
                if (bottom != 0 ) {
                    ...
                    make_stud(box_w1, 61, 0, 4,pcb_h,stud_l);
                    ...
                    translate(v=[box_w2-14      ,box_l1,1])  cube([3.0, 4,pcb_h-1.1]);
			...
			// besides rca support
                    translate(v = [box_w2-stud_w, box_l1+Crca_y+Crca_d+1 - 1.5, pcb_c+0.4]) {
                        cube([stud_w, 4, box_h-pcb_c-1], center = false);
                    }
                }
            } // color
