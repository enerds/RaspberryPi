 ...
 * but this one includes a minimal game of life
 */
...
/*
_cs   = PD2; // _cs on arduino digital 2
_rs   = PD1; // _rs on arduino digital 1
_rst  = PD0; // reset on arduino digital 0
*/
...
void show(void *u, int w, int h){
    ...
    for_y {
        for_x {
            if(univ[y][x]){
                fillRect(x*8,y*8,8,8,Color565(255,255,255));
            }
        }
    }
}
...
int main(void){
  ...
  fillScreen(ST7735_BLACK);
    ...
    while(1){
        ...
        while(count < 1000){
            count++;
            if(x_up){x+=3;}else{x-=2;}
            if(y_up){y+=2;}else{y--;}
            drawPixel(x,y,Color565(255,255,255));
            if(x >= 127) x_up = 0;
            if(y >= 159) y_up = 0;
            if(x <= 0) x_up = 1;
            if(y <= 0) y_up = 1;
